...
let _pendingReceiptMime = 'image/jpeg';
...
document.addEventListener('click', async (event) => {
  ...
  try {
    if (target.id === 'add-tx-fab') {
      ...
      return;
    } else if (target.dataset.receiptTx) {
...
(async function init() {
  ...
  // Receipt upload
  document.getElementById('receipt-upload-btn')?.addEventListener('click', () => {
    document.getElementById('modal-receipt-file')?.click();
  });
