...
async function runSearch(q) {
  ...
  try {
    ...
    if (!rows.length) {
      results.innerHTML = '<div class="search-empty">找不到符合的交易</div>';
      return;
    }
    ...
    results.innerHTML = '<div class="search-empty">搜尋失敗</div>';
  }
}
...
async function loadCalendar() {
  ...
  document.getElementById('cal-title').textContent = `${year}/${String(month).padStart(2,'0')}`;
  try {
    const data = await api(`/api/calendar?year=${year}&month=${month}`);
    renderCalendarGrid(data);
  } catch { document.getElementById('calendar-grid').innerHTML = '<div class="text-muted">載入失敗</div>'; }
}
...
function renderCalendarGrid(data) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const expenseDisplay = info && info.expense > 0
    ...
    html += `<div class="cal-cell${isToday ? ' cal-today' : ''}${info ? ' cal-has-tx' : ''}" data-cal-date="${dateStr}">
      ...
      ${expenseDisplay ? `<div class="cal-expense">${expenseDisplay}</div>` : ''}
    </div>`;
  }
  html += '</div>';
  grid.innerHTML = html;
...
function showCalDayDetail(date, info) {
  ...
  if (info.count) lines.push(`<div class="cal-tx-row text-muted" style="font-size:11px"><span>共 ${info.count} 筆</span><button class="link-btn" data-goto-date="${date}">查看明細 →</button></div>`);
...
async function checkBudgetOverspend(category) {
  ...
  try {
    ...
    if (spent > limit && limit > 0) {
      ...
      showBudgetWarning(category, over, spent, limit);
    }
  } catch { /* ignore */ }
}
...
document.querySelector('#tx-modal-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (state.modalType === 'expense') {
      checkBudgetOverspend(cat).catch(() => {});
    }
    closeTxModal();
    showStatus('已新增交易');
    await refreshCurrentView();
...
// Calendar cell clicks (delegated)
document.getElementById('calendar-grid')?.addEventListener('click', e => {
  const cell = e.target.closest('[data-cal-date]');
  if (!cell) return;
  const date = cell.dataset.calDate;
  showCalDayDetail(date, _calDayMap.get(date));
});
// "查看明細" button in calendar detail
document.addEventListener('click', e => {
  if (e.target.dataset.gotoDate) {
